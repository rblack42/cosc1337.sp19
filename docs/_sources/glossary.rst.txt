...
..  glossary::
    ...
    Cloud
        The funny new term we use to refer to machines "out ther" somewhere on the Internet.
    ...
    variable name
        A human friendly name we pick to :term:`refer` to a piece of data in
        memory.   
    ...
    address
        A simple number used to identify where in memory we want something to
        happen., In most computers, each byte of memory has a unique numerical
        address.
    ...
    constant
        A memory location whose value is not supposed to change during the
        execution of a program.

    data type
        Specifies the size of a data container needed to store values, and the
        rules to use to manipulate that data.    

    flow chart
        A diagram of the ``flow`` of a program through decision and process
        blocks. A decision is shown in a diamond block and a process in a
        rectangle.

    desk checking     
        Thinking your way through a piece of code manually. You record the
        values of all variables involved, and determining exactly how the code
        will ``flow``.

    baby steps
        A programming style where you make small changes to a working program
        to get another working program that is closer to the final solution. Do
        not move on until the new program works! 

    blasting code
        A term used to indicate long sessions furiously writing code in the
        hope you can get things running by a deadline. It is the exact opposite
        of :term:`baby steps` programming!   

    strongly typed
        A language like C++ forces you to declare all variables and set the
        :term:`data type` at compile time. You cannot change the type once it
        is defined. Python does not work this way. A data container can hold
        anything, and the type can change as you wish.

    symbol table
        All language processing tools (compilers) maintain a table of user
        named defined in the program. When you refer to a name, the symbol
        table is consulted to make sure what you are doing is legal in the
        language you are using.

    scope
        This term refers to where you can use a name in your code. Typical
        scopes are file-level, procedure level, or block level. 

    block
        A program block is a sequence of lines of code treated as a unit.
        Typical blocks are surrounded in C++ with curly braces, and are
        indented in Python.
